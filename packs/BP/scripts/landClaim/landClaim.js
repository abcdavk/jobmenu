...
export class Expired {
    ...
    init(player, block) {
        ...
        const expiredAt = now + twelveDaysInMs;
        const newEntry = {
    ...
    update(player) {
        const now = Date.now();
        const twelveDaysInMs = 12 * 24 * 60 * 60 * 1000;
        const newExpireTime = now + twelveDaysInMs;
        const currentData = this.getAllExpiredDate();
...
system.afterEvents.scriptEventReceive.subscribe((ev) => {
    if (ev.id === "lc:setting") {
        ...
        if (ev.message === "get") {
            ...
            const exp = world.getDynamicProperty("lc:expired");
            console.log(data);
            console.log(exp);
        }
    }
...
function handleShowAllPlayers(player) {
    let rawData = new Protection().getAll();
    let form = new ActionFormData()

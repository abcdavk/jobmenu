import { BlockComponent, Block, BlockPermutation, BlockStateType, Entity, EntityComponentTypes, Player, system, world, Vector3 } from "@minecraft/server";
import { ActionFormData } from "@minecraft/server-ui"

interface KillEntityOptions {
    entity: Entity, 
    reward: number,
}
...
function calculateNextValue(level: number, initial: number, step: number): number {
...
function giveReward(player: Player, reward: number) {
    let playerJob = player.getDynamicProperty("job:currentJob") as number;
    let playerMoney = player.getDynamicProperty("job:money") as number;
    let jobTitle = allJobs[playerJob].title;
    let jobProgress = player.getDynamicProperty(`job:${jobTitle}_progress`) as number;
...
world.afterEvents.playerBreakBlock.subscribe(({
    brokenBlockPermutation: blockPerm,
    player
}) => {
    let playerJob = player.getDynamicProperty("job:currentJob") as number;
    if (!playerJob) return;
